        ...
        try {
            ...
            if (provider === "login") {
                const result = await this.userRepository.findOne({
                    where: {
                        [Op.and]: [{ Users_email: userLoginRequestDTO.userEmail }, { Users_provider: "local" }],
                    },
                });

                if (!result) throw new BadRequest("이메일 혹은 비밀번호를 확인해 주세요.");
                ...
                if (!isPasswordCorrect) throw new BadRequest("아이디 혹은 비밀번호를 확인해주세요.");
